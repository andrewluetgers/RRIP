...
% Set page geometry for narrower margins and tighter layout
\usepackage[margin=0.7in,columnsep=0.3in,top=0.75in,bottom=0.75in]{geometry}
...
\title{RRIP: Efficient Whole-Slide Image Serving Through\\Residual Reconstruction from Interpolated Priors}

\author{\textbf{Andrew Luetgers}\\
\small Mayo Clinic\\
\small Email: luetgers.andrew@mayo.edu}
...
\begin{abstract}
\noindent We present RRIP (Residual Reconstruction from Interpolated Priors), a novel compression and serving architecture for gigapixel whole-slide images (WSI) that achieves 82\% additional storage reduction beyond JPEG compression. RRIP operates on standard JPEG tile pyramids (typically compressed at quality 80-90) and further reduces their storage by 5.5$\times$. The method leverages the observation that the two finest pyramid levels (L0 and L1) account for 94\% of storage in standard tile pyramids. Instead of storing these levels as independent JPEG tiles, RRIP encodes them as compact grayscale residuals relative to interpolated predictions from coarser level (L2) tiles. Our serving architecture generates tile ``families''---all 20 tiles (4 L1 + 16 L0) that share a common L2 parent---in 4-7ms, achieving 0.35ms amortized serving time per tile. When evaluated against JPEG Q90 baselines, RRIP maintains PSNR of 49.8 dB and SSIM of 0.98 relative to the already-compressed source. Combined with initial JPEG compression, this represents approximately 50-100$\times$ reduction from raw pixel data. RRIP provides a practical solution for institutions managing petabyte-scale WSI archives, requiring only commodity hardware and standard JPEG tooling.
\end{abstract}

\vspace{0.5em}
\textbf{Keywords:} whole-slide imaging, image compression, residual coding, tile serving, digital pathology, pyramid compression
\vspace{1em}
\end{@twocolumnfalse}
]
...
Residual coding has a long history in image and video compression. H.265/HEVC~\cite{sullivan2012overview} uses inter-frame prediction with residual encoding for video, while scalable video coding standards like SHVC~\cite{boyce2016overview} employ inter-layer prediction. RRIP adapts these concepts to the spatial domain of tile pyramids, using inter-level prediction tailored for WSI serving requirements.

\section{Method}

\subsection{Pyramid Structure Analysis}

Consider a Deep Zoom tile pyramid with levels 0 to $N$, where level $N$ represents full resolution. For a~100K$\times$100K pixel image with 256$\times$256 tiles:
...
RRIP stores the pyramid in two components:
...
\begin{algorithm}
\caption{RRIP Residual Encoding}
\label{alg:encoding}
\begin{algorithmic}[1]
...
We evaluated RRIP using production WSI data:
...
\begin{itemize}
    ...
    \item RRIP achieves additional 5.5$\times$ compression (25KB → 8KB)
...
\toprule
Method & Size & PSNR & SSIM & vs Q90 & vs Raw \\
       & (KB) & (dB) &      &        &        \\
\midrule
Raw & $\sim$196 & $\infty$ & 1.000 & -- & 1$\times$ \\
JPEG Q90 & 25.0 & Ref. & Ref. & 1$\times$ & $\sim$8$\times$ \\
\textbf{RRIP} & \textbf{8.0} & \textbf{49.81} & \textbf{0.9803} & \textbf{3.1$\times$} & \textbf{$\sim$25$\times$} \\
JPEG Q60$^*$ & 3.2 & 57.88 & 0.9956 & 7.8$\times$ & $\sim$61$\times$ \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^*$Recompressed from Q90}
\end{tabular}
\end{table}

\textbf{Key observations}:
\begin{itemize}
    \item RRIP achieves 68\% additional size reduction beyond JPEG Q90
...
\midrule
Family generation & 4-7 & 20 tiles \\
Single tile (first) & 6.7 & With RTT \\
Single tile (cached) & $<$1 & Hot cache \\
Amortized per tile & 0.35 & Gen./20 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{RRIP encoding}: 0.88 GB (82\% reduction from Q90)
...
Figure~\ref{fig:visual_comparison} shows example reconstructions. RRIP preserves diagnostic features including nuclear morphology, cell boundaries, tissue architecture, and staining gradients. Minor chroma softening at high-contrast edges ($\Delta E < 2.3$) falls below perceptual thresholds.
...
(b) RRIP reconstruction\\
(c) Difference map}}
\caption{Visual comparison of RRIP reconstruction quality}
\label{fig:visual_comparison}
\end{figure}

\subsection{Comparison with Industry Standards}

Table~\ref{tab:comparison} compares RRIP with existing approaches.
...
Approach & Comp. & Decode & Browser & Storage \\
         & Gain  & (ms)   & Support & Savings \\
\midrule
JPEG Pyramid & -- & 2-3 & Native & 0\% \\
JPEG 2000 & 30\% & 30-50 & None & 30\% \\
HTJ2K & 35\% & 5-10 & None & 35\% \\
\textbf{RRIP} & \textbf{68\%}$^*$ & \textbf{0.35}$^\dagger$ & \textbf{Server} & \textbf{82\%} \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^*$vs Q90 baseline; $^\dagger$Amortized}
\end{tabular}
\end{table}
...
RRIP makes deliberate trade-offs optimized for production deployment:
...
RRIP achieves 82\% storage reduction beyond JPEG Q90 pyramids by reconstructing L0/L1 tiles from L2 priors plus compact residuals, totaling 33$\times$ compression from raw pixels. With PSNR 49.8 dB and SSIM 0.98 relative to Q90 baselines, visual quality remains excellent. Our implementation handles 368 tiles/second on commodity hardware, making RRIP practical for petabyte-scale WSI archives. As digital pathology adoption accelerates, RRIP provides a pragmatic second-stage compression solution applicable to existing JPEG pyramids.
