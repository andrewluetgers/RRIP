# Evaluation Methodology for RRIP Paper (JPEG Source)
...
Due to the practical constraints of whole-slide imaging systems, where slides are typically captured and stored as JPEG-compressed pyramids, we evaluate RRIP using production-quality JPEG tiles as our reference baseline. While this differs from traditional codec evaluations that use lossless sources, it reflects the real-world deployment scenario where RRIP would be applied to existing digital pathology archives.
...
We compare RRIP against two industry-standard codecs:
...
3. **RRIP Residual Compression:** Our proposed method
...
*Note: Sizes are per-tile averages. RRIP size includes both L2 reference and residuals.*
...
| **RRIP** | **37.5** | **0.948** | **3.2 ms** | **0.35 ms*** |
...
RRIP's unique family-based reconstruction provides significant advantages:
...
| **RRIP** | 7 ms (first) | 22 ms (cached) | **368** |
...
RRIP shows degraded performance in:
...
- RRIP's 56% reduction remains significant even with JPEG source
...
Despite using JPEG-compressed references rather than lossless sources, RRIP demonstrates:
