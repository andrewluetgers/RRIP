...
pub fn encode_wsi(
    ...
    for batch_start in (0..families.len()).step_by(config.batch_size) {
        ...
        for &(col, row) in batch {
            ...
            // --- l1_residuals ---
            let t0 = Instant::now();
            let l1_tile_w = tile_w;
            let l1_tile_h = tile_h;
    ...
    // Generate comprehensive README manifest
    let timestamp = chrono::Utc::now().to_rfc3339();
    let families_with_data = families_encoded - timers.families_empty;
    let l2_width = grid_cols * 256;
    let l2_height = grid_rows * 256;
    let readme = format!(
r#"# ORIGAMI WSI Encoding Report

## Overview
This directory contains a DZI-compatible pyramid encoding of a whole-slide image.

**Generated**: {}
**Source**: {}
**Encoder**: origami-gpu-encode (GPU-accelerated)

## Image Specifications
- **Dimensions**: {}×{} pixels (Level 0 grid coverage)
- **Tile Size**: {}×{} pixels
- **Grid**: {}×{} families ({} total positions)
- **Families with data**: {} ({:.1}%)
- **Empty families**: {} ({:.1}%)

## Compression Settings
- **L2 Baseline Quality**: Q{} (JPEG, {} chroma)
- **L1 Residual Quality**: Q{}
- **L0 Residual Quality**: Q{}
- **OptL2 Optimization**: {}
{}
## Encoding Performance
- **GPU Encoding Time**: {:.2}s
- **Throughput**: {:.1} families/s ({:.2} ms/family)
- **Pyramid Generation**: {} (CPU, {} cores)

## File Sizes
- **L2 Baseline**: {:.1} MB ({} tiles)
- **L1+L0 Residuals**: {:.1} MB
- **Total Encoded Data**: {:.1} MB
{}
## Directory Structure

### DZI-Compatible Viewing
- `slide.dzi` - DeepZoom Image manifest (point viewers here)
- `slide_files/` - Tile pyramid directory
  - `slide_files/0/` - Level 0 (L2 baseline, highest resolution)
  - `slide_files/1/` - Level 1 (L3, first pyramid level)
  - `slide_files/2-9/` - Levels 2-9 (L4-L11, down to 1×1 thumbnail)

### Metadata
- `summary.json` - Machine-readable encoding statistics
- `README.md` - This file
...
### Pyramid Structure
- Level 0 contains the original L2 baseline tiles (encoded on GPU)
- Levels 1-9 are generated via CPU box filter + unsharp mask (strength={:.2})
- Each level is downsampled by 2× until reaching a single 1×1 tile
- Empty regions in the source are preserved in the pyramid

### Quality vs Size Trade-offs
The quality settings balance file size with reconstruction accuracy:
- Higher L2 baseq = better baseline quality but larger files
- Lower residual quality = smaller files but higher reconstruction error
- OptL2 optimization trades encoding time for better prediction (smaller residuals)

---
Generated by ORIGAMI GPU Encoder v1.0
https://github.com/andrewluetgers/RRIP
"#,
        timestamp,
        dicom_path.display(),
        l2_width, l2_height,
        ...
        families_encoded,
        families_with_data,
        families_with_data as f64 / families_encoded as f64 * 100.0,
        timers.families_empty,
        timers.families_empty as f64 / families_encoded as f64 * 100.0,
        config.baseq,
        if config.subsamp == "444" { "4:4:4" } else { "4:2:0" },
        config.l1q,
        config.l0q,
        if config.optl2 { "enabled" } else { "disabled" },
        if config.optl2 {
            format!("- **Max Delta**: ±{} pixel values\n", config.max_delta)
        } else {
            String::new()
        },
        elapsed,
        families_encoded as f64 / elapsed,
        elapsed * 1000.0 / families_encoded as f64,
        if config.generate_pyramid { "Generated" } else { "Not generated" },
        rayon::current_num_threads(),
        total_l2_bytes as f64 / 1_048_576.0,
        families_with_data,
        total_residual_bytes as f64 / 1_048_576.0,
        (total_l2_bytes + total_residual_bytes) as f64 / 1_048_576.0,
        if config.generate_pyramid {
            format!("- **Pyramid Tiles**: {} (estimated, levels 1-9)\n- **Total Output**: ~{:.0} MB (with pyramid)\n",
                    131*181 + 66*91 + 33*46 + 17*23 + 9*12 + 5*6 + 3*3 + 2*2 + 1*1,
                    (total_l2_bytes + total_residual_bytes) as f64 / 1_048_576.0 + 120.0)
        } else {
            String::new()
        },
    ...
    info!("Generated README.md manifest");
...
fn generate_dzi_pyramid_cpu(
    ...
    // Generate DZI manifest for DeepZoom compatibility
    let dzi_manifest = format!(
        r#"<?xml version="1.0" encoding="utf-8"?>
<Image TileSize="{}" Overlap="0" Format="jpg" xmlns="http://schemas.microsoft.com/deepzoom/2008">
  <Size Width="{}" Height="{}"/>
</Image>"#,
        tile_size, l2_width, l2_height
    );

    // Rename directory to DZI convention: baseline_pyramid_files -> slide_files
    let slide_files_dir = files_dir.parent().unwrap().join("slide_files");
    if files_dir.exists() {
        fs::rename(files_dir, &slide_files_dir)?;
    }

    // Write DZI manifest
    let dzi_path = files_dir.parent().unwrap().join("slide.dzi");
    fs::write(&dzi_path, dzi_manifest)?;
    info!("Created DZI manifest: {}", dzi_path.display());
